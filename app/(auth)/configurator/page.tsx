...
function ConfiguratorPage() {
    ...
    const handleSaveCroppedImage = async () => {
        try {
            ...
            const file = new File([croppedBlob], 'cropped-print.jpg', { type: 'image/jpeg' });
    ...
    const calculatePrice = () => {
        let basePrice = 8500;
        if (layout === '100') basePrice += 3000;
        if (printFile) basePrice += 2000;
    ...
    const handleSendAssembly = async () => {
        ...
        try {
            ...
            alert('Ошибка сети. Проверь, запущен ли бэкенд на localhost:5237');
        } finally {
            setIsSending(false);
        }
    };
                    ...
                    >
                        <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-3xl h-[550px] flex flex-col overflow-hidden shadow-2xl">
                            <div className="p-5 border-b border-white/5 flex justify-between items-center bg-zinc-950">
                                <div>
                                    <h3 className="text-sm font-bold font-mono text-purple-400 uppercase tracking-wider">📐 Центрирование и подгонка принта</h3>
                                    <p className="text-[11px] text-zinc-400 mt-0.5">Настрой положение картинки внутри рамки</p>
                                </div>
                            ...
                            <div className="flex-1 relative bg-zinc-950/40">
                                <Cropper
                                    ...
                                    aspect={cropAspectRatio} onCropChange={setCrop}
            ...
            >
                <div>
                    <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
                        ...
                        {editMode && (
                            <button onClick={() => window.location.href = '/profile'} className="text-xs text-zinc-500 hover:text-white font-mono">
                                ← НАЗАД
                            </button>
                        )}
                    </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between bg-zinc-950 p-2.5 rounded-lg border border-white/5">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-xs font-mono max-w-[140px] truncate text-zinc-300">{printFile?.name}</p>
                                            <p className="text-[9px] text-emerald-400 font-bold font-mono">ПОДОГНАН ПО РАЗМЕРУ</p>
                                        </div>
                                    </div>
                    ...
                    <div className="mb-5 p-4 rounded-xl bg-white/[0.02] border border-white/5">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {caseColorsList.map(c => (
                                <button type="button" key={c} onClick={() => setCaseColor(c)} className={`w-7 h-7 rounded-lg border transition-all ${caseColor === c ? 'border-white scale-110' : 'border-white/10'}`} style={{ backgroundColor: c }}></button>
                            ))}
                        </div>
                    </div>
                    ...
                    >
                        <label className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mb-3 block">Цвет клавиш (Keycaps)</label>
                        <div className="flex flex-wrap gap-2">
                            {keycapColorsList.map(c => (
                                <button type="button" key={c} onClick={() => triggerKeycapWave(() => setKeycapColor(c))} className={`w-7 h-7 rounded-lg border transition-all ${keycapColor === c ? 'border-white scale-110' : 'border-white/10'}`} style={{ backgroundColor: c }}></button>
                            ))}
                        </div>
                    </div>
                    <div className="mb-5 p-4 rounded-xl bg-white/[0.02] border border-white/5 transition-all hover:border-zinc-700"
                    ...
                    >
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {['linear', 'tactile', 'clicky'].map(type => (
                                <button type="button" key={type} onClick={() => handleSwitchTypeChange(type as any)} className={`py-2 border text-center rounded-lg text-[9px] font-mono transition ${switchType === type ? 'border-amber-400 bg-amber-400/10 text-amber-400' : 'border-white/5 text-zinc-400'}`}>{type.toUpperCase()}</button>
                            ))}
                        </div>
                    </div>
                    <div className="mb-5 p-4 rounded-xl bg-white/[0.02] border border-white/5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {[
                                ...
                                <button type="button" key={mat.id} onClick={() => setKeycapMaterialType(mat.id)} className={`py-2 text-[9px] font-mono border rounded-lg ${keycapMaterialType === mat.id ? 'border-cyan-400 text-cyan-400' : 'border-white/5 text-zinc-400'}`}>{mat.name}</button>
                            ))}
                        </div>
                    </div>

                    <div className="flex gap-2 border-t border-white/5 pt-4 pb-4">
                        <button type="button" onClick={() => setRevealSwitches(!revealSwitches)} className={`flex-1 py-3 text-[10px] font-mono rounded-xl border ${revealSwitches ? 'bg-amber-500 text-black font-bold' : 'border-zinc-800 text-zinc-400'}`}>
                            {revealSwitches ? '⏬ Собрать клавиатуру' : '💥 Взрыв-Схема'}
                        </button>
                ...
                <div className="border-t border-white/5 pt-4 bg-zinc-950 p-4 rounded-xl border border-white/5">
                    <div className="flex justify-between items-baseline mb-4">
                        <span className="text-[10px] text-zinc-500 uppercase font-mono">ИТОГ:</span>
                        <span className="text-3xl font-black tracking-tight font-mono text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">{calculatePrice()}</span>
                    </div>
                    <button type="button" onClick={handleSendAssembly} disabled={isSending} className="w-full py-4 bg-gradient-to-r from-purple-500 to-cyan-500 text-black font-black uppercase tracking-widest text-xs hover:opacity-90 transition-all rounded-xl disabled:opacity-50">
                ...
                {Studio ? (
                    <Studio.Canvas shadows camera={{ position: [0, 3.8, 5.2], fov: 34 }} dpr={[1, 2]} gl={{ antialias: true, alpha: false, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 0.85 }}>
                        ...
                        <Suspense fallback={null}>
                            ...
                            <KeyboardEngine
                                Studio={Studio} layout={layout} caseColor={caseColor} keycapColor={keycapColor}
                                volumeColor={volumeColor} switchColor={switchColor} keycapMaterialType={keycapMaterialType}
                                revealSwitches={revealSwitches} rgbMode={rgbMode} hoveredSection={hoveredSection}
                                waveResetTrigger={waveResetTrigger} printBlobUrl={printBlobUrl}
                            />
...
function KeyboardEngine({ Studio, layout, caseColor, keycapColor, volumeColor, switchColor, keycapMaterialType, revealSwitches, rgbMode, hoveredSection, waveResetTrigger, printBlobUrl }: any) {
    ...
    useEffect(() => {
        ...
        const minBound = globalBox.min;
        const maxBound = globalBox.max;
        ...
        scene.traverse((child: any) => {
            ...
            if (geometry && geometry.attributes.position) {
                ...
                for (let i = 0; i < posAttr.count; i++) {
                    ...
                    const u = (tempWorldVertex.x - minBound.x) / totalSizeX;
                    const v = (tempWorldVertex.z - minBound.z) / totalSizeZ;
                    customUvArray[i * 2] = 1 - u;
                    customUvArray[i * 2 + 1] = v;
                }
        ...
        scene.traverse((child: any) => {
            ...
            if (type === 'keycap') {
                ...
                if (currentWaveTime >= 0 && currentWaveTime < 1.5) {
                    const spatialIndex = child.userData.waveIndex ?? 0;
                    const waveDelay = spatialIndex * 0.012;
                    const phase = (currentWaveTime - waveDelay) * 10;
                ...
                mats.forEach((mat: any) => {
                    ...
                    if (keycapMaterialType === 'clear' && !currentTexture.current) { mat.transparent = true; mat.opacity = 0.45; }
                    else { mat.transparent = false; mat.opacity = 1.0; }
                });
            }
                ...
                mats.forEach((mat: any, index: number) => {
                    ...
                    const isStem = matName.includes('stem') || matName.includes('shtok') || (type === 'switch_stem_mesh') || (mats.length > 1 && index === 1);
                    if (isStem) applyMaterialSettings(mat, switchColor, 0.35, 0.0, 0.4);
                    else {
                        applyMaterialSettings(mat, '#1a1a1e', 0.5, 0.1, 0.3);
                        if (mat.emissive) {
                            if (rgbMode) { const hue = (time * 0.3 + child.position.x * 0.05) % 1; mat.emissive.setHSL(hue, 1, 0.5); mat.emissiveIntensity = 2.5; }
