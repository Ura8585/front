...
export default function HomePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const catalogRes = await fetch('https://kdbackend.ryban.ru/api/catalog');
                if (catalogRes.ok) setCatalog(await catalogRes.json());

                const galleryRes = await fetch('https://kdbackend.ryban.ru/api/gallery');
                if (galleryRes.ok) {
        ...
        const checkAuth = async () => {
            ...
            if (!token) return;
            try {
                const response = await fetch('https://kdbackend.ryban.ru/api/users/profile', {
    ...
    const handleAuth = async () => {
        ...
        try {
            if (authMode === 'login') {
                const res = await fetch('https://kdbackend.ryban.ru/api/auth/login', {
                    method: 'POST',
                ...
                if (res.ok && data.token) {
                    ...
                    const profileRes = await fetch('https://kdbackend.ryban.ru/api/users/profile', {
                ...
                const res = await fetch('https://kdbackend.ryban.ru/api/auth/register', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
