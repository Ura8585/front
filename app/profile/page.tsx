...
export default function ProfilePage() {
    ...
    const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:5237/api/users/upload-avatar', {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
                body: formData,
            });
    ...
    const handleCancelOrder = async (orderId: number) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:5237/api/orders/${orderId}/cancel`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (!response.ok) throw new Error('Ошибка отмены');

            const ordersRes = await fetch('http://localhost:5237/api/orders/my-orders', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const handlePublishToGallery = async (configId: number) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:5237/api/gallery/publish', {
                method: 'POST',
                headers: {
                    ...
                    <div className="flex flex-col md:flex-row gap-10 items-start">
                        <div className="flex flex-col items-center">
                            <div className="relative group w-40 h-40 rounded-3xl overflow-hidden border-4 border-white/10 shadow-2xl">
                                {user?.avatarUrl ? (
                                    <img src={`http://localhost:5237${user.avatarUrl}`} alt="Avatar" className="w-full h-full object-cover" />
                                ) : (
